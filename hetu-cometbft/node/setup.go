...
func createMempoolAndMempoolReactor(
	...
	case cfg.MempoolTypeNarwhal:
		logger = logger.With("module", "narwhal_mempool")

		// Create the original mempool for block proposals
		originalMp := mempl.NewCListMempool(
			config.Mempool,
			proxyApp.Mempool(),
			state.LastBlockHeight,
			mempl.WithMetrics(memplMetrics),
		...
		originalMp.SetLogger(logger)

		// Create Narwhal client
		narwhalClient := mempl.NewDefaultNarwhalClient(config.Narwhal.Addr, logger)
		// Do not start the client here, it will be started by NarwhalMempool

		// Create Narwhal mempool that wraps the original
		mp := mempl.NewNarwhalMempool(narwhalClient, originalMp)
		mp.SetLogger(logger)

		// Start the mempool
		if err := mp.Start(); err != nil {
			panic(fmt.Sprintf("failed to start narwhal mempool: %v", err))
		}

		// Create original reactor with proposal mempool
		originalReactor := mempl.NewReactor(config.Mempool, mp.ProposalMempool())
		originalReactor.SetLogger(logger)

		// Create Narwhal reactor and wire it with mempool
		reactor := mempl.NewNarwhalMempoolReactor(mp, originalReactor)
		reactor.SetLogger(logger)
		mp.SetReactor(reactor)
